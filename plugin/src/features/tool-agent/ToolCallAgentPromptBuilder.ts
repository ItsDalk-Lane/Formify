...
export class ToolCallAgentPromptBuilder {
	build(input: ToolAgentPromptInput): ToolAgentPrompt {
		...
		const systemPrompt = [
			...
			'- If file, vault, search, or directory tools are provided, assume Vault access is available through those tools.',
			'- Do not claim that you lack file-system or vault access unless no relevant read/search/list tool is provided.',
			'- When the task is to inspect, search, summarize, compare, or read vault content, try an applicable provided tool before returning needs_clarification.',
