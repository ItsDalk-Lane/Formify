...
export const getMaxTriggerLineLength = (settings: TarsSettings) => {
...
export class TagEditorSuggest extends EditorSuggest<TagEntry> {
	...
	async selectSuggestion(element: TagEntry, _evt: MouseEvent | KeyboardEvent) {
		...
		try {
			...
			if (!provider) {
				const providerName = element.tag.startsWith('#') ? element.tag.substring(1) : element.tag
				throw new Error(`${t('Provider not configured')}: ${providerName}`)
			}
