...
import { Toast } from "src/component/toast/Toast";
...
import { CommandIdConflictDetector } from "src/service/variable/CommandIdConflictDetector";
  ...
  private showToast(result: ScanResult) {
    ...
    const onFixAll = async () => {
      ...
      if (result.commandIdConflicts.length > 0) {
        await CommandIdConflictDetector.fixConflicts(this.app.vault, result.commandIdConflicts);
      }

      // 2) 修复变量冲突（逐文件）
      for (const [filePath, conflicts] of result.variableConflictsByFile.entries()) {
        await VariableConflictAutoFixer.fixFileConflicts(this.app.vault, filePath, conflicts);
      }
    ...
    toastRef = ToastManager.warning(
      React.createElement(ConflictToastContent, {
