...
从 0.1.1 版本开始，字段的默认值（以及用户在字段中输入的任何值）支持使用内置变量。这些变量会在表单提交时自动替换为实际的值。

支持的内置变量包括：

- `{{date}}` - 当前日期，格式：`2025-01-01`
- `{{date:YYYY-MM-DD}}` - 自定义格式的日期
- `{{time}}` - 当前时间，格式：`12:00:00`（包含秒）
- `{{time:HH:mm:ss}}` - 自定义格式的时间（包含秒）
- `{{time:HH:mm}}` - 自定义格式的时间（不含秒）
- `{{clipboard}}` - 剪贴板内容
- `{{selection}}` - 当前编辑器中选中的文本

**使用示例：**

1. 在文本字段的默认值中输入：`{{date}} 的笔记`
   - 提交时会自动替换为：`2025-01-01 的笔记`
...
3. 组合使用：`今天是 {{date}}，现在是 {{time}}`

> **注意**：变量的替换发生在表单提交时，这意味着 `{{date}}` 和 `{{time}}` 等时间相关变量会使用提交时的时间，而不是表单打开时的时间。

更多关于内置变量的详细说明，请查看[内置变量](./03_form_action/05_internal_variable.md)章节。
...
时间字段支持输入「时、分、秒」三个时间单位，格式为 `10:00:00`
